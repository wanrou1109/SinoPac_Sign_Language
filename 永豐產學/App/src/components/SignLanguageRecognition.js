            ...
            try {
                ...
                const camera = new Camera(videoRef.current, {
                    onFrame: async () => {
                        ...
                        // 確認 video 存在且畫面已準備好（readyState >= 2）
    ...
    const sendFrameToRecognition = async () => {
        try {
            ...
            if (data.success === true && data.text && data.text.trim() !== '') {
                ...
                console.log('跳過無效結果');
                if (data.success !== true) console.log('  - success 不為 true:', data.success);
                if (!data.text) console.log('  - text 為空或不存在:', data.text);
                if (data.error) console.log('  - 錯誤信息:', data.error);
            }
        } catch (err) {
